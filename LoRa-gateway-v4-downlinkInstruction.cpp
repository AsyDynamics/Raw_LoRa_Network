...
void loop() {
  while (!getLocalTime( & timeinfo)) {
    ...
    // just before the BeaconPeriod, check if any instruction from server
    // if instruction, construct downlink message with timeinfo
    // instruction maybe 
...
void onReceive(int packetSize) {
  ...
  // switch(sensorMode): provide multiple case to read and save LoRa.read() data
  // and later send to thingsboard via telemetry
...
void sendMessage() {
  ...
  // LoRa.write(numOfInstruction); // provide number of instruction
  // for (byte i = 0; i< numOfInstruction) // write the nodeID and instructions into lora
  LoRa.endPacket();
