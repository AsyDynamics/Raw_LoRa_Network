...
const byte BeaconPeriod = 2;        // 2 min, BP, interval for sync time
const byte SensePeriod = 10;        // 10 sec, SP, interval for collecting sensor data
...
void loop() {
  //*************** BeaconPeriod *************************
  bool sync_cond1 = minute() % BeaconPeriod == 1 && second() > 58; // sync time window [59,
  bool sync_cond2 = minute() % BeaconPeriod == 0 && second() < 2; // sync time window 1]
