...
byte drift_min =
    floor(localAddress / 10.0);  // 1-10(current minute), 11-20(next minute)
byte drift_sec = (localAddress - floor(localAddress / 10.0)) *
                 5;  // 1-10 corresponds to 5,10,15,20...50 sec
bool syncFlag = false;
...
void setup() {
  ...
  syncTime();
  syncFlag = false; // reset syncFlag for later useage
  LoRa.sleep();
  digitalWrite(A2, LOW);
}
...
void loop() {
  //*************** BeaconPeriod *************************
  bool sync_cond1 =
      minute() % BeaconPeriod == 1 && second() > 55;  // sync time window [59,
  bool sync_cond2 =
      minute() % BeaconPeriod == 0 && second() < 5;  // sync time window 1]
  if (sync_cond1 || sync_cond2) {
    syncTime(8*1000); // may be locked here, the timeout break is uncommented now
  }

  //*************** SensePeriod *************************
  bool send_cond1 =
      second() >= drift_sec && (second() - drift_sec) % SensePeriod == 0;
  if (second() == 10 || second() == 35 || second() == 50) {  // time drift as ID*5
    sendMessage();
    delay(600);
  }
    
  // after SensePeriod, wait a little for downlink message? or just go sleep
}
...
//////////////////////////////////////////////////////
void syncTime(int threshould = 125*1000) {
  ...
  while (!syncFlag) {
    if (LoRa.parsePacket() != 0) {
      ...
      if ((recipient == localAddress || recipient == 0xFF) &&
          (group == groupID)) {  // check length for error
        ...
        syncFlag = true;
        // break; or return; but maybe not necessary
      }
    }
      if (millis()-initialTime > threshould){
          break;
      }
  }
  digitalWrite(A2, LOW);
...
void sendMessage() {
  ...
  byte bmeTint, bmeTdec, bmeHint, bmeHdec, bmePint, bmePdec, bmeAint, bmeAdec,
      max1int, max1dec, max2int, max2dec;
  if (bme280.measure()) {
    bmeT = bme280.getTemperature() / 100.0;
    bmeH = bme280.getHumidity() / 1024.0;
    bmeP = bme280.getPressure() / 1000.0;
    bmeA = 44330.0 *
           (1.0 - pow((bme280.getPressure() / 100.0F) / 1013.25, 0.1903));
  }
  ...
  // LoRa.write(sensorMode); // the sensor mode to indicate what sensors in this node
  // A-2xPT100, B-indoorTemp, C-humid, D-leakage, multiple combination/mode
  LoRa.write(bmeTint);
  LoRa.write(bmeTdec);
  LoRa.write(bmeHint);
  LoRa.write(bmeHdec);
