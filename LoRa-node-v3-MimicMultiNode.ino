...
const byte localAddress = 1;     // address of this device
const byte destination = 0;      // destination to send to
const byte groupID = 0;          // group ID to distinguish between groups
...
void loop() {
  ...
  if ( second()%5==0 && second()<52 ) {  
    byte nodeAddress = (minute()%BeaconPeriod)*10 + second()/5;
